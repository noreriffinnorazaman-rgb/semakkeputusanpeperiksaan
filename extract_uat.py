"""
Extract Ujian Awal Tahun (UAT) marks from all 5 Tingkatan Excel files
into spm_data.json, computing rankings (kedudukan dalam kelas & tingkatan).
"""
import openpyxl
import json
import os
...
def extract_uat():
    ...
    uat_data = {}      # class_name -> list of student records
    guru_kelas = {}    # class_name -> guru name
    subject_maps = {}  # tingkatan -> {code: full_name}
    class_subjects_by_form = {}  # tingkatan -> {class_name: [subject_full_names]}

    for tingkatan in [1, 2, 3, 4, 5]:
        fn = f'UJIAN AWAL TAHUN TINGKATAN {tingkatan} 2026 (1).xlsx'
        if not os.path.exists(fn):
            print(f'  SKIP: {fn} not found')
            continue

        print(f'\n{"="*60}')
        print(f'  EXTRACTING TINGKATAN {tingkatan}: {fn}')
        print(f'{"="*60}')
        ...
        for r in range(2, 30):
            ...
            if code and name:
                name = name.replace('\n', ' ').strip()
                subj_map[code] = name
        subject_maps[tingkatan] = subj_map
        ...
        for sheet_name in class_sheets:
            ...
            # Identify subject mark columns (even cols are grades, odd are marks)
            # Subject cols have a corresponding grade col at col+1
            ...
            class_subjects_by_form.setdefault(tingkatan, {})[sheet_name] = class_subject_names
    ...
    # Group by tingkatan for tingkatan ranking
    tingkatan_groups = {}  # tingkatan -> list of all students
    for cls_name, students in uat_data.items():
        for s in students:
            t = s['tingkatan']
            tingkatan_groups.setdefault(t, []).append(s)

    # Rank within each class (by GP ascending - lower GP is better)
    for cls_name, students in uat_data.items():
    ...
    # Rank within each tingkatan (by GP ascending)
    ...
    data['guru_kelas'] = guru_kelas
    data['subject_maps'] = {str(k): v for k, v in subject_maps.items()}
    data['class_subjects_by_form'] = {str(k): v for k, v in class_subjects_by_form.items()}

    # Also update marks_data for compatibility with existing marks entry system
    ...
    for cls_name, students in uat_data.items():
        ...
        for s in students:
            ...
            data['marks_data'][cls_name][sname]['UAT'] = {}
            for subj_name, subj_data in s['subjects'].items():
                mark = subj_data.get('mark', '')
                data['marks_data'][cls_name][sname]['UAT'][subj_name] = mark
    ...
    print(f'{"="*60}')
    total = 0
    for cls_name in sorted(uat_data.keys()):
        students = uat_data[cls_name]
        total += len(students)
        top = min(students, key=lambda x: x.get('gp', 99)) if students else None
        print(f'  {cls_name}: {len(students)} students | Top: {top["name"][:30] if top else "-"} (GP={top["gp"]:.2f}, KDK={top["kdk"]}, KDT={top["kdt"]})')
    print(f'\n  TOTAL: {total} students')
    print(f'  Guru Kelas: {len(guru_kelas)} entries')
